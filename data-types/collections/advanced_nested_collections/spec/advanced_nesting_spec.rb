...
RSpec.describe 'Advanced Nested Collections' do
  ...
  it 'test 6' do
    ...
    dishes_names = stores[:olive_garden][:dishes].map{|name| name[:name]}
  ...
  it 'test 7' do
    ...
    # all restaurants
    employee_names = []
    stores.each do |store, key|
      employee_names << key[:employees]
    end
  ...
  it 'test 8' do
    ...
    ingredients = []
    stores.each do |store, key|
      ingredients << key[:dishes].map{|ingredients| ingredients[:ingredients]}
    end
  ...
  xit 'test 11' do
    ...
    stores.each do |store|
      dish_name = store.fetch(:dishes, :name)
      dish_hash = store.dig(:dishes)
      full_menu << dish_name.zip(dish_hash).to_h
    end
